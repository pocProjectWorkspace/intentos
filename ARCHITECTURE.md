...
│   one isolated profile per OS user                   │
├─────────────────────────────────────────────────────┤
│                  INTENT KERNEL                       │
...
├─────────────────────────────────────────────────────┤
│            SEMANTIC MEMORY LAYER (RAG)               │
│    (user context, file index, task history)          │
├─────────────────────────────────────────────────────┤
│         AGENT COMMUNICATION PROTOCOL (ACP)           │
│        (standard contract between all agents)        │
├─────────────────────────────────────────────────────┤
│           intentos-daemon (system user)              │
│   acts on filesystem only within granted paths       │
└─────────────────────────────────────────────────────┘
          │  ACL-scoped access only
          ▼
  HOST OS FILESYSTEM
  ~/Documents  ~/Downloads  ~/intentos-workspace
```
...
Every interaction with IntentOS is scoped to a specific human OS user. The User Identity Layer is the boundary between the human's session and everything IntentOS does on their behalf. It ensures that on a shared machine, John's files, tasks, RAG index, and preferences are completely invisible to Sarah's IntentOS instance — even though the same `intentos-daemon` serves both.

**How it works:**

When a task arrives, the User Identity Layer loads the profile for the currently logged-in OS user before anything else runs. All subsequent operations — Intent Kernel context, RAG retrieval, agent execution, file access — are scoped exclusively to that user's profile and granted paths.

**Profile location (per OS user):**
```
~/.intentos/
├── profile.json          ← preferences, identity, settings
├── grants.json           ← which folders intentos-daemon may access
├── rag/                  ← personal RAG indexes (file, task, profile)
├── task_history.jsonl    ← append-only task log
└── workspace/
    ├── outputs/          ← files created by agents
    ├── temp/             ← working files during execution
    └── exports/          ← files ready for the user
```

**The daemon model:**

`intentos-daemon` is a system-level service running under a dedicated OS user (`intentos` on Linux, `_intentos` on macOS, `INTENTOS_SVC` on Windows). It has no interactive login, no desktop access, and no default file permissions. It can only touch paths the human user has explicitly granted via `grants.json`, which is written during first launch and editable any time from Settings.

This means agents never run with the human user's full privileges. See `/docs/SECURITY.md` for the full security model and platform-specific implementation details.

**Responsibilities:**
- Load and validate the OS user's IntentOS profile on every task
- Scope all RAG queries, task history, and preferences to the active user
- Enforce that `intentos-daemon` only accesses granted paths
- Isolate profiles completely on shared machines

**Technology:** JSON profile store + platform ACL management (setfacl / macOS Privacy grants / Windows ACLs).
...
### 8. Semantic Memory Layer (RAG System)
The context engine of IntentOS. Instead of a hierarchy of bytes, it is a queryable store of everything the user has — their files, their task history, and their inferred preferences. It is what allows the Intent Kernel to understand vague references, resolve ambiguity, and get smarter about how a specific user works over time.

**Three indexes, one context layer:**
- **File Index** — every file on the filesystem, semantically embedded. Understands content, not just filenames.
- **Task Index** — every completed task stored with full context. Enables replay, pattern detection, and history queries.
- **User Profile Index** — inferred preferences passively built from usage. Date formats, preferred folders, frequent contacts, avoided actions.

**On every task, the Context Assembler queries all three indexes and injects the most relevant context into the Intent Kernel prompt before any inference happens.** This is what turns *"the invoice from Ahmed"* into a specific file path without the user ever specifying it.

**Responsibilities:**
- Index filesystem content with semantic embeddings (priority-based, background)
- Watch filesystem for changes and update index in real time
- Store and embed completed task history
- Maintain and update user preference profile passively
- Assemble and inject context into Intent Kernel on every task

**Technology:** ChromaDB (embedded, no server) + nomic-embed-text via Ollama + watchdog filesystem watcher.

**All index data is local only. Nothing is ever transmitted externally. See `/docs/RAG_SYSTEM.md` for full design.**
...
Phase 1 — Proof of Concept
  ...
  ✦ Inference Router v0 (local SLM via Ollama)

Phase 2 — Core Capabilities + RAG v1
  ...
  ✦ RAG: File Index (filesystem semantic search)

Phase 3 — OS Experience + RAG v2
  ✦ Task Interface v1 (desktop GUI)
  ✦ Capability Registry + auto-install
  ✦ System Agent
  ✦ Document Agent
  ✦ ACP standardization
  ✦ RAG: Task Index (history, replay, pattern detection)

Phase 4 — Platform + RAG v3
  ✦ Public capability registry (IntentHub)
  ✦ Contributor SDK
  ✦ IntentOS distributable image
  ✦ RAG: User Profile Index (preference learning, proactive suggestions)
```
